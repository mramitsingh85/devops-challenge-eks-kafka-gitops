...
do
    ...
    do
        ...
        # Wait for AWS-managed ENIs (EKS, ELB, etc.)

        echo "Waiting for in-use ENIs to disappear..."

        for WAIT in $(seq 1 20)
        do
            ENI_COUNT=$(aws ec2 describe-network-interfaces \
              --filters Name=vpc-id,Values=$VPC \
              --query 'length(NetworkInterfaces[?Status==`in-use`])' \
              --output text \
              --region "$AWS_REGION")

            [ "$ENI_COUNT" = "0" ] && break

            echo "In-use ENIs remaining: $ENI_COUNT"
